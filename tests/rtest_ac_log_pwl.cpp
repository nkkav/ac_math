...
// ==============================================================================
// Test Designs
//   These simple function allow executing the ac_log_pwl() function.
...
void test_ac_log_pwl_float(
  const ac_float<Wfl, Ifl, Efl, AC_TRN> &in,
  ac_float<outWfl, outIfl, outEfl, AC_TRN> &log_out
)
{
  log_out = ac_log_pwl<ac_float<outWfl, outIfl, outEfl, AC_TRN> >(in);
}
...
//   piece-wise linear base e logarithm model with the computed base e logarithm
...
)
{
  ...
  for (ac_fixed<Wfi + 2, Ifi + 1, false, AC_TRN, AC_WRAP> i = lower_limit; i <= upper_limit; i += step) {
    ...
    test_ac_log_pwl_fixed(input,log_out);

    double actual_log = log_out.to_double();
    double this_error_log;

    // Calculate absolute value of error for log output. Since the scaling of outputs that lie outside the range of normalization of log2 is done
    ...
    if (this_error_log > max_log_error) { max_log_error = this_error_log; }
  }

  if (max_log_error > cumulative_max_error_fixed) { cumulative_max_error_fixed = max_log_error; }

  passed = (max_log_error < allowed_error);

  if (passed) { printf("PASSED , max err (%f)\n", max_log_error); }
  else        { printf("FAILED , max err (%f)\n", max_log_error); } // LCOV_EXCL_LINE
...
)
{
  ...
  for (int i = 0; i < exp_arr_size; i++) {
    ...
    for (ac_int<Wfl - 1, false> mant_i = lower_limit_it; mant_i <= upper_limit_it; mant_i += step_it) {
      ...
      T_out output_float;
      test_ac_log_pwl_float(input_float, output_float);
      double actual_float = output_float.to_double();
      double expected_float = T_out(log(input_float.to_double())).to_double();
